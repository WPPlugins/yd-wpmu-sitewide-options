...
function yd_wpmuso_display_dbtable( $table, $options, $i ) {
	$tbl = 'ydtable_' . esc_attr( $table );
	$tbl2 = 'yddata_' . esc_attr( $table );
	$tbld = esc_attr( $table );
	$opt_id = 'table_' . $table;
	$opt2_id = 'data_' . $table;
	echo '<tr>';
	    		...
	    		if( preg_match( '/^ydtable_/', $opt ) ) {
	    			if( $options[$i]['overwrite_data'] ) {
	    				foreach( $blog_list as $blog ) {
		    				...
		    				yd_wpmuso_create_table( $opt, $blog['blog_id'] );
	    				}
	    			}
	    		} elseif( preg_match( '/^yddata_/', $opt ) ) {
	    			if( $options[$i]['overwrite_data'] ) {
	    				foreach( $blog_list as $blog ) {
		    				if( $blog['blog_id'] == $master_blog_id ) continue;
		    				yd_wpmuso_replicate_data( $opt, $blog['blog_id'], true );
	    				}
	    			}
	    		} else {
	    			$newoption[$opt] = $value;
...
function yd_wpmuso_options_new_blog_action( $blog_id ) {
	...
	**/
	if( $options[$i]["auto_new_blog"] && is_array( $options[$i]["selected_options"] ) ) {
		foreach( $options[$i]["selected_options"] as $opt ) {
			if( preg_match( '/^ydtable_/', $opt ) ) {
				yd_wpmuso_create_table( $opt, $blog_id );
    		} elseif( preg_match( '/^yddata_/', $opt ) ) {
				yd_wpmuso_replicate_data( $opt, $blog_id, true );
    		} else {
				//echo $opt . ': ';
				$value = false;
...
function yd_wpmuso_create_table( $opt, $blog_id ) {
	global $wpdb;
	//echo $opt . '<br/>';
...
function yd_wpmuso_replicate_data( $opt, $blog_id, $delete = false ) {
    global $wpdb;
